...
int main(int argc, char *argv[]) {
    ...
    std::vector<pair<double, unsigned >> query_dist;
...
#pragma omp parallel for num_threads(60)
    for (unsigned cur_count = 1; cur_count <= count_bound; cur_count++) {
        ...
        while (ans1 <= 0) {
            left = rand() % attr_length + 1;
            right = rand() % attr_length + 1;
            if (left > right) swap(left, right);
            u = rand() % graph.weight_graph_.size();
            v = rand() % graph.weight_graph_.size();
        ...
        if (!double_equal(ans2, ans3))
            std::cerr << "ERROR:: " << u << " " << v << " " << ans1 << " " << ans2 << endl;
        query_dist[cur_count].first = time_slap;
    ...
    for (int i = 1; i <= 10; i++) {
        for (int j = 1; j <= count_bound / 10; j++) {
            ...
            split_time_tree[i] += tree_time[idx];
            split_time_bi_dij[i] += bi_dijkstra_time[idx];
            split_time_dij[i] += dijkstra_time[idx];
            cur++;
        }
        split_time_tree[i] /= ((double) count_bound / 10);
        split_time_bi_dij[i] /= ((double) count_bound / 10);
        split_time_dij[i] /= ((double) count_bound / 10);
    }
